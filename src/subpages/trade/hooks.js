  ...
  const tradeActionBtns = {
    ...
    LOGISTICS: {
      ...
      action: ({ orderId, isAllDelivery, ordersDeliveryId, deliveryCorpName, deliveryCode }) => {
        if (isAllDelivery) {
          Taro.navigateTo({
            url: `/subpages/trade/delivery-info?delivery_corp_name=${encodeURIComponent(deliveryCorpName || '')}&delivery_code=${encodeURIComponent(deliveryCode || '')}&delivery_id=${ordersDeliveryId}`
          })
        } else {
          Taro.navigateTo({
