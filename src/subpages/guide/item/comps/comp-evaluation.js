  ...
  return (
    <View className={classNames('comp-evaluation', className)}>
      <View className='evaluation-hd'>
        <View className='title'>{ti('64b33113.0c834e', [list.length])}</View>
        {list.length > 0 && (
        ...
        {list.map((item) => (
          <View className='evaluation-item-wrap'>
            <View className='item-hd'>
              ...
              <Text className='evaluation-name'>{item.username || $t('64b33113.708229')}</Text>
            </View>
            <View className='evaluation-content'>{item.content}</View>
          </View>
        ))}
        {list.length == 0 && <View className='default-msg'>{$t('64b33113.5a8497')}</View>}
      </View>
    </View>
  )
}
