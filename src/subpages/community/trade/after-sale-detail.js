...
import S from '@/spx'
import CompAfterDetailItem from '../comps/comp-afterdetailitem'
...
class TradeDetail extends Component {
  ...
  getParamsAboutItem = (oldInfo, isInit = false) => {
    ...
    if (isInit) {
      is_has_point =
        newInfo.orders && newInfo.orders.reduce((total, item) => total + item.remain_point, 0) > 0
      is_has_normal =
        newInfo.orders && newInfo.orders.reduce((total, item) => total + item.remain_fee, 0) > 0

      newOrders =
        newInfo.orders &&
        newInfo.orders.map((item) => ({
          ...item,
          /** 单个商品的积分 */
    ...
    let total_point_money =
      newOrders &&
      newOrders.reduce((total, i) => total + (i.is_checked ? i.selected_point_total : 0), 0)

    let total_normal_money =
      newOrders &&
      newOrders.reduce((total, i) => total + (i.is_checked ? i.selected_normal_total : 0), 0)
  ...
  async fetch() {
    ...
    sessionFrom += `"商品": "${info.orders && info.orders[0].title}"`
    sessionFrom += `"订单号": "${info.orders && info.orders[0].order_id}"`
    sessionFrom += '}'
    const _this = this
    this.setState({
  ...
  handleCopy = async () => {
    ...
    const msg = [
      ti('1d9cdff5.5b020e', [info.receiver_name, info.receiver_mobile]),
  ...
  async handleClickBtn(type, val) {
    ...
    if (type === 'REFUND') {
      ...
      Taro.navigateTo({
        url: `/subpages/community/trade/refund?deliverData=${deliverData}&status=${type}&order_id=${info.tid}`
      })
    }
  }
  ...
  handleLookDelivery = (value) => {
    let { info } = this.state
    Taro.navigateTo({
      url: `/subpages/community/trade/split-bagpack?order_type=${info.order_type}&order_id=${info.tid}`
    })
  }
        ...
        <ScrollView scroll-y className='content' scrollIntoView={scrollIntoView}>
          ...
          {info.is_logistics && (
            <View className='screenZiti' id='order-1'>
              ...
              <View className='trade-detail-goods'>
                <CompAfterDetailItem info={info} showType='log_orders' />
              </View>
            </View>
          )}
