...
class TradeDetail extends Component {
  ...
  render() {
    const { list, deliverycorp, deliverycode, deliveryname, LOGISTICS_CODE } = this.state
    if (!list) {
    ...
    return (
      <View className='delivery-detail'>
        <SpNavBar title={$t('b8a3efcc.24bc07')} leftIconType='chevron-left' fixed='true' />
        <View className='delivery-detail__status'>
          <View className='delivery-detail__status-text'>
            {ti('b8a3efcc.da0a9d', [
              deliveryname !== 'underfined' && deliveryname !== 'null' ? deliveryname : ''
            ])}
          </View>
          <View className='delivery-detail__status-ordertext'>
            {ti('b8a3efcc.5a58a0', [
              deliverycode !== 'undefined' && deliverycode !== 'null' ? deliverycode : ''
            ])}
          </View>
        ...
        <View className='delivery-info'>
          {list.length === 0 ? (
            <SpNote img='plane.png'>{$t('b8a3efcc.6e5fd6')}</SpNote>
          ) : (
