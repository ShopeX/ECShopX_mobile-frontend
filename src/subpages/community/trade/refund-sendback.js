...
//   'JTSD': '极兔速递',
// }
class TradeRefundSendback extends Component {
  ...
  componentDidMount() {
    Taro.setNavigationBarTitle({ title: $t('d6fd4742.55a413') })
    this.initState()
  }
  ...
  handleSubmit = async () => {
    ...
    if (!corp_code) {
      S?.toast($t('da10dc51.08faf4'))
      return
    }

    if (!logi_no) {
      S?.toast($t('da10dc51.6a74f5'))
      return
    }
          ...
          <View className='sec'>
            <View className='at-input'>
              <View className='at-input__container'>
                <View className='at-input__title'>{$t('da10dc51.f21828')}</View>
                <Picker
                  mode='selector'
                  range={express}
                  rangeKey='corp_name'
            ...
            <AtInput
              title={$t('da10dc51.22a25f')}
              name='logi_no'
              type='text'
              placeholder={$t('da10dc51.6a74f5')}
              value={logi_no}
...
export default withTranslation()(TradeRefundSendback)
