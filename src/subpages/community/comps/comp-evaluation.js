  ...
  return (
    <View className={classNames('comp-evaluation', className)}>
      <View className='evaluation-hd'>
        <View className='title'>{ti('b1ac6d7a.0c834e', [list.length])}</View>
        {list.length > 0 && (
      ...
      <View className='evaluation-bd'>
        {list.map((item, idx) => (
          <View className='evaluation-item-wrap' key={idx}>
            <View className='item-hd'>
              <SpImage src={item.avatar} className='evaluation-icon' width={50} height={50} />
              <Text className='evaluation-name'>{item.username || $t('b1ac6d7a.708229')}</Text>
            </View>
            <View className='evaluation-content'>{item.content}</View>
          </View>
        ))}
        {list.length == 0 && <View className='default-msg'>{$t('b1ac6d7a.5a8497')}</View>}
      </View>
    </View>
  )
}
